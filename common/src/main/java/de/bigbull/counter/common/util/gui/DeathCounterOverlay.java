...
public class DeathCounterOverlay {
    ...
    public static void render(GuiGraphics guiGraphics) {
        ...
        if (!ConfigHelper.get().getServer().showDeathOverlay() || !ConfigHelper.get().getServer().enableDeathCounter()) return;

        boolean isEditMode = (minecraft.screen instanceof OverlayEditScreen);
        boolean allowListOverlay = client.showDeathListOverlayAlways() || CounterManager.isTabPressed() || isEditMode;
        boolean allowSelfOverlay = client.showDeathSelfOverlayAlways() || CounterManager.isTabPressed() || isEditMode;
        ...
        if (isMultiplayerGame(minecraft)) {
            switch (ConfigHelper.get().getServer().deathOverlayMode()) {
        ...
        boolean showList = (client.showDeathListOverlayAlways() && client.showDeathListOverlay())
                || isEditMode
                || (CounterManager.isTabPressed() && serverWantsList && client.showDeathListOverlay());

        boolean showSelf = (client.showDeathSelfOverlayAlways() && client.showDeathSelfOverlay())
                || isEditMode
                || (CounterManager.isTabPressed() && serverWantsSelf && client.showDeathSelfOverlay());
        ...
        int listX = (int) (client.deathListOverlayX() * screenWidth);
        int listY = (int) (client.deathListOverlayY() * screenHeight);
        float listScale = (float) client.deathListOverlaySize();

        int selfX = (int) (client.deathSelfOverlayX() * screenWidth);
        int selfY = (int) (client.deathSelfOverlayY() * screenHeight);
        float selfScale = (float) client.deathSelfSize();
    ...
    private static void drawSelfOverlay(GuiGraphics guiGraphics, int x, int y, float scale, LocalPlayer player, boolean isEditMode) {
        int color = client.deathSelfTextColor();
    ...
    private static void drawListOverlay(GuiGraphics guiGraphics, int x, int y, boolean isEditMode) {
        ...
        int defaultTextColor = client.deathListTextColor();
        int backgroundColor = 0x80000000;
        int boxWidth = calcDeathListWidth();
        int boxHeight = calcDeathListHeight();

        if (client.deathListOverlayStyle() == IClientConfig.DeathListOverlayStyle.TABLE) {
            drawTableOverlay(guiGraphics, x, y, sortedDeaths, isEditMode);
        } else if (client.deathListOverlayStyle() == IClientConfig.DeathListOverlayStyle.CLASSIC) {
            if (client.deathListOverlayStyle() == IClientConfig.DeathListOverlayStyle.BOXED) {
            ...
            Counter.logger.warn("Unbekannte Einstellung für DEATH_OVERLAY_STYLE: {}", client.deathListOverlayStyle());
    ...
    private static void drawStatusAndBorder(GuiGraphics guiGraphics, int x, int y, int width, int height, boolean isEditMode, OverlayEditScreen.DragTarget target) {
        ...
        boolean isEnabled = switch (target) {
            case DEATH_LIST -> client.showDeathListOverlay();
            case DEATH_SELF -> client.showDeathSelfOverlay();
            default -> false;
        };
    ...
    public static Component getDeathEntry(int index, String playerName, int deaths) {
        ...
        if (index == 0) {
            positionColor = client.firstPlaceColor();
        } else if (index == 1) {
            positionColor = client.secondPlaceColor();
        } else if (index == 2) {
            positionColor = client.thirdPlaceColor();
        }
    ...
    public static int calcDeathListWidth() {
        Minecraft mc = Minecraft.getInstance();
        float scale = (float) client.deathListOverlaySize();
        int maxTextWidth = client.deathListOverlayWidth();
    ...
    public static int calcDeathSelfWidth() {
        ...
        float scale = (float) client.deathSelfSize();
